...
class TestDatasetLoader(unittest.TestCase):
    ...
    def setUp(self):
        ...
        svg_content = (
            '<svg xmlns="http://www.w3.org/2000/svg"><path d="M10 10 L 90 90"/></svg>'
        )
    ...
    def test_create_and_load_dataset(self):
        ...
        expected_moj_path = os.path.join(
            self.processed_path_parent, f"{dataset_name}.moj"
        )
        self.assertTrue(os.path.exists(expected_moj_path))

        # 3. Test loading from the existing .moj file
        loaded_dataset_list = create_or_load_dataset(
            self.raw_path, force_recreate=False
        )
