...
def generate_handwriting(args):
    ...
    with torch.no_grad():
        for char in args.text:
            ...
            full_svg_content += (
                f'<g transform="translate({current_x_offset}, 0)">\n{svg_path}\n</g>\n'
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate handwriting from a trained model."
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    parser.add_argument(
        "--text", type=str, required=True, help="The text string to generate."
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default="output/svg/generated.svg",
        help="Path to save the output SVG, PNG, or JPG file.",
    )
    parser.add_argument(
        "--random_variation",
        type=float,
        default=DEFAULT_RANDOM_VARIATION,
        help="Amount of random noise to add for style variation.",
    )
    parser.add_argument(
        "--stroke_width",
        type=float,
        default=DEFAULT_STROKE_WIDTH,
        help="Base width of the stroke.",
    )
